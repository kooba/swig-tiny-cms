...
exports.canEditContent = function (isAuthorized) {
...
exports.compile = function (compiler, args, content, parents, opts, blockName) {
  ...
  var filePath = path.join(options.contentDirectory, contentId + '.md');
 ...
 * @param {object} options - Swig Tiny-CMS options.
 */
exports.initialize = function (swig, app, opts) {
  ...
  app.get('/' + options.route + '/edit/:contentId', function (req, res) {
    ...
    var content = '';
    var filePath = path.join(options.contentDirectory, req.params.contentId + '.md');
  ...
  app.post('/' + options.route + '/save', function (req, res) {
    ...
    if (!fs.existsSync(options.contentDirectory)) {
      mkdirp.sync(options.contentDirectory);
    }
  ...
  watch(options.contentDirectory, function () {
    swig.invalidateCache();
  });
};
...
var validateOptions = function (opts) {
  ...
  if (!opts.contentDirectory)
    throw new TypeError('Content directory for Swig Tiny-CMS is not defined.');

  if (!fs.existsSync(opts.contentDirectory)) {
    mkdirp.sync(opts.contentDirectory);
  }
  ...
  if(opts.markedOptions)
    marked.setOptions(opts.markedOptions);
