...
exports.parse = function (str, line, parser) {
  parser.on('end', function () {
    if (!this.out[0] || this.out[0] === '')
      throw new Error('Swig CMS tag requires ContentId argument e.g. {% cms \'contentId\' %}');
  });
  return true;
};
...
exports.initialize = function (swig, app, options) {
  ...
  app.post('/' + swigCmsOptions.route + '/save', function (req, res, next) {

    if (admin) {
      if (req.body && req.body.content && req.body.contentId && req.query.returnUrl) {
        ...
        next(new Error('Swig CMS Error: Insufficient parameters when saving content.')); //Letting user handle the error.
      }
    } else {
      res.redirect(req.query.returnUrl);
    }
